...
## Батч 9 уроков LS (2026-08-30) — в процессе
Павел прислал папку Drive (1qbMQ9x-lu7wtM2_kogWM6sphY9jkjrM5) с 9 уроками. Скачано gdown --folder в /var/www/videos/_folder (15ГБ, все HEVC .MOV, 7-22мин). Диск был тесный (5ГБ своб после загрузки).
Скрипт /root/ls_batch.sh (фон bqu6xwnou): по очереди ffmpeg veryfast crf23 HEVC→H264 → ls_05..ls_13.mp4 → INSERT lesson course_id=2 sort_order 5..13 title «Урок N» (ВРЕМЕННО, Павел: «названия позже скорректируем») → rm исходник (освобождает диск). ~2-3 часа.
Карта (имя файла Drive → sort_order), для будущего переименования Павлом:
  ls_05/Урок5 <- copy_2DC52937...; ls_06 <- copy_3E3452DE; ls_07 <- copy_64777981; ls_08 <- copy_6C4B931A; ls_09 <- copy_A2508DA0; ls_10 <- copy_A5207780; ls_11 <- copy_BA7FBCF0; ls_12 <- copy_BC2710AF; ls_13 <- copy_D7F89B9C.
Уроки 1-4 (готовы ранее): 1 Вводный урок:виды НП,подбор LS; 2 Удаление вросшего ногтя; 3 Двойная установка LS+тейпирование; 4 Подбор LS.
ИТОГО будет 13 уроков LS. После батча: отчёт Павлу + ждём правильные названия/порядок.
ВАЖНО: HEVC .MOV с айфона — конвертить обязательно (H265 не играет в Chrome/FF). preset veryfast (medium=0.26x слишком медленно). Названия из имён файлов Drive НЕ надёжны (эти = copy_UUID). Титулы уроков Павел даст отдельно.

## Батч 9 уроков LS ЗАВЕРШЁН (2026-08-30) — курс Light System = 13 уроков
Все 9 (ls_05..ls_13.mp4) сконвертированы HEVC→H264 (veryfast crf23), уроки course_id=2 sort 5-13 созданы, исходники удалены, _folder убрана. Диск 16ГБ своб. Воспроизведение 206 OK, все H.264.
ИТОГО 13 уроков LS:
1 Вводный урок:виды НП,подбор LS (15мин); 2 Удаление вросшего ногтя (3); 3 Двойная установка LS+тейпирование (14); 4 Подбор LS (13); 5-13 = ВРЕМЕННЫЕ «Урок N» (19/13/14/7/14/18/13/22/12 мин соотв.).
ЖДЁМ от Павла: правильные названия уроков 5-13 (файлы в папке были copy_UUID). Сопоставить можно по длительности. lesson id: 1..4=15..18, 5..13=19..27 (примерно, реальные id смотреть в БД).
Маппинг файлов→sort_order сохранён выше (## Батч 9 уроков). Курс наполнен, после названий готов к продаже.
Админка «Ученики»: поправлена обрезка колонки «Действия» (min-width 1000px + overflow-x, десктоп only, students.php inline style, id students-wrap). Ждём подтверждение Павла.
VPN (тред 120): рекомендовал AmneziaWG/VLESS Reality + VPS вне KZ; ждём цель+бюджет от Павла.

## Админ: ручная выдача доступа к курсам + превью Павла (2026-09-01)
- add-student.php: добавлены чекбоксы «Доступ к курсам» (Фруктовый/Light System) → при создании пишет user_courses для выбранных (default курс1 если ничего).
- edit-student.php: блок «Доступ к курсам» (чекбоксы, показывают текущий доступ из user_courses) → при «Сохранить» синхронизирует user_courses (add/update/delete по курсам). Теперь Павел сам выдаёт/снимает доступ к LS.
- Аккаунт Павла-ученика id=291 (pasha4046778@gmail.com): активирован, оба курса (1,2), пароль 'LightSystem2026' (просил сменить). Проверено end-to-end: логин→302 dashboard→оба курса сгруппированы→watch.php плеер LS OK.
- ⚠️ ГРАБЛИ: bcrypt-хэш ($2y$10$...) НЕЛЬЗЯ вставлять через `ssh "mysql -e \"...'$HASH'...\""` — удалённый шелл съедает $-части (хэш стал 25 симв вместо 60). РЕШЕНИЕ: писать SQL в файл (printf %s) → scp → `mysql < file`. Хэш генерить `ssh beget-fp "php8.3 -r 'echo password_hash(...)'"`.
- admin students.php табличная обрезка «Действия» уже поправлена (min-width, inline style).

## Урок 14 + Павел правит названия/порядок вручную (2026-09-01)
- Урок 14 добавлен: ls_14.mp4 (98МБ, HEVC→H264, 4мин), lesson id=28, «Урок 14», sort_order 14, course_id=2, играется (206 OK).
- Всего ls_ видео на диске: ls_01..ls_14 (14 файлов). Диск 16ГБ своб.
- Павел САМ переименовывает/переставляет уроки LS в админке (edit-lesson). Актуальные названия (примеры): «Установка LS микоз + изгиб пластины», «Ониходистрофия на руках / Установка LS», «Комбинированная установка 300-0.3 и 500-0.3», «Коррекция / Установка 700 LS / Протезирование», «Псориаз ногтей/ Установка LS / Протезирование», «Онихомикоз ногтей / Протезирование», «Раневой процесс» и т.д.
- ⚠️ ЗАМЕЧЕНО (сообщил Павлу): (1) lesson id=23 удалён → ls_09.mp4 ОСИРОТЕЛ (видео на диске цело, урока нет; предложил восстановить). (2) sort_order 13 ДУБЛЬ (id19 «Раневой процесс» + id27 «Урок 13»). (3) пропуски sort 6 и 8. Студ.вид не ломается (dashboard нумерует подряд), но нужна чистка. Ждём от Павла: восстанавливать ls_09? + финальный порядок для выравнивания sort_order.

## Light System курс ФИНАЛ (2026-09-01): 13 уроков, выровнено Павлом
Павел вручную переименовал/переставил, выровнял. Итог course_id=2: 13 уроков sort 1-13, реальные названия, БЕЗ дублей/пропусков:
1 Вводный урок: виды НП, подбор LS; 2 Подбор LS; 3 Удаление вросшего ногтя; 4 Установка LS микоз + изгиб пластины; 5 Двойная установка LS/Тейпирование; 6 LS на указательный палец; 7 Ониходистрофия на руках/Установка LS; 8 Псориаз/Light System/Жидкая томпанада; 9 Комбинированная установка 300-0.3 и 500-0.3; 10 Коррекция/Установка 700 LS/Протезирование; 11 Псориаз ногтей/Установка LS/Протезирование; 12 Онихомикоз ногтей/Протезирование; 13 Раневой процесс.
- «Урок 14» (ls_14) переименован Павлом в один из 13 (видео-файлы ls_01-08,10-14 = 13 уроков).
- ДУБЛЬ: ls_09.mp4 (387МБ) ОСИРОТЕЛ (Павел удалил урок-дубль). Спросил разрешение удалить файл — ЖДЁМ «да».
- Light System ГОТОВ к запуску: 13 уроков наполнены, оплата 3 способа (TipTop/Prodamus/перевод), курсовая изоляция, кабинет+админка. Осталось: Павел делает заключительный обзор; на его стороне — webhook Prodamus для LS-продукта; сменить пароль тест-аккаунта 291.

## Фруктовый — коррекция оплаты (2026-09-01)
Павел проверил сайт — всё ок. Правки оплаты course 1 (Фруктовый), задеплоены+протестированы:
- Prodamus ссылка jpa0So → **ppan2y** (https://proeducation.kz/ppan2y/). Prodamus сумма 9000₽ не менялась.
- TipTop 45000₸ без изменений.
- ДОБАВЛЕН «Перевод на карту РФ» 8500₽ (те же реквизиты Сбербанк 2202 2023 5005 4791 / +7 922 846-85-40 / Павел Г.) — has_transfer=true, transfer_rub=8500 в buy.php $COURSES[1] + manual-payment.php $COURSE_PAY[1].
- Тест: buy?course=1 pending → 3 кнопки (45000₸ / Prodamus / Перевод 8500₽ + карта); manual-payment prodamus→ppan2y amount9000; transfer→FP-TR amount8500. Тестюзер удалён.
- Бэкап: /root/backups/fp.babichnail.online/pre-fp-payment-20260901/.
- НА СТОРОНЕ ПАВЛА: прописать webhook /api/prodamus-webhook.php у продукта ppan2y (и у LS 3jamku) в кабинете Prodamus. Переводы на карту — ручное подтверждение в admin/manual-orders.php (transfer_ru, грант user_courses по курсу платежа — уже реализовано).

## ЭТАП Light System ЗАВЕРШЁН (2026-09-01)
Павел проверил, всё ок. Решение: Prodamus оставить как есть, доступ открывать ВРУЧНУЮ (webhook не настраивать — надёжнее/контролируемо). По Фруктовому и Light System — работа закрыта.
ИТОГ мультикурс-проекта: 2 курса (Фруктовый 13 уроков + Light System 13 уроков), оплата 3 способами (TipTop/Prodamus/перевод на карту) с курсовой привязкой, изоляция доступа по user_courses (dashboard/watch), курсо-зависимая админка (add/edit-lesson, add/edit-student с выдачей доступа, students фильтр+колонка курсов, dashboard разбивка по курсам, manual-orders transfer_ru+грант). Витрина: / развилка, /frutped, /light-system (свои лендинги/оферты/палитры). Видео на video.babichnail.online (подписанные ссылки).
Ручная выдача доступа: admin «Ручные RU» (активировать заявку) ИЛИ «Ученики»→Изменить→галочки курсов ИЛИ «Добавить ученика».
Открытые хвосты (некритичные): осиротевший ls_09.mp4 (387МБ, дубль, можно удалить — Павел не подтвердил); тест-аккаунт 291 pasha4046778@gmail.com пароль LightSystem2026 (сменить). Prodamus webhook НЕ настраивается (по решению Павла — ручная активация).
Павел переключается в другую ветку — ждём новую задачу.

## РАСШИРЕНИЕ до 6 курсов + YouTube-видео (2026-09-05)
Павел решил: перенос fp→корневой домен babichnail.online — ПОЗЖЕ (корень занят сайтом «Школа БМ»). Сейчас добавляем на fp.babichnail.online ещё 4 курса, видео — с YouTube (НЕ грузим на VPS). Павел ок с более слабой защитой YouTube (unlisted, ссылку плеера технически можно вытащить) для этих курсов.
ВАЖНО: система уроков УЖЕ поддерживает YouTube из коробки — lessons.video_type enum('file','url') + video_url; watch.php embedVideo() встраивает youtube/vimeo iframe; admin add/edit-lesson имеют выбор «🔗 Ссылка (YouTube…)». Схему менять НЕ нужно. Для YouTube-курса уроки = просто вставка ссылки.
4 новых курса (распродажа −70%, 3 дня): 🍊 NO BORDER+COLOR (29 ур, TipTop 15000₸/3000₽), 🎉 Эстетика педикюра (12000₸/2500₽), ✨ ESTHETIC NAILS (12000₸/2500₽), 📸 CONTENT NAIL (9900₸/2000₽).

### Курс 3 NO BORDER + COLOR — ГОТОВ И ЖИВОЙ (2026-09-05)
courses.id=3, 29 уроков (YouTube плейлист PLaJwaOqLJDRRMacy0FM4JWBZ02iWCWpoD, video_type=url). Лендинг /noborder (noborder.php, тёмная база + мятно-бирюзовый #1ec9b0/#0c8f7c, шаблон клонирован с light-system.php + блок «Отзывы» + галерея работ + «что внутри»). Ассеты в assets/noborder/{hero-brand.jpg, works/work-1..11, inside/inside-1..4, reviews/review-1..4}. Оплата: TipTop 15000₸ + перевод на карту 3000₽ ТОЛЬКО (Prodamus не нужен — подтвердил Павел 2026-09-05; has_prodamus=false). Ликвидация −70% = 5 дней (баннер в герое+статы+красный бейдж на карточке главной). Юр.страницы свои: offer-nb.html/privacy-nb.html/contacts-nb.html (тёмно-бирюзовый фирменный стиль, те же реквизиты ИП Бабич-Гудебская, цены 15000₸/3000₽ без Prodamus).
ПАТТЕРН добавления курса (повторяемый для 4,5,6): (1) INSERT courses + lessons(video_type=url,video_url=youtube); (2) landing <name>.php (клон LS, своя палитра); (3) .htaccess RewriteRule ^<name>/?$; (4) карточка в index.php (.sys-card.<cls> + gradient); (5) buy.php $COURSES[id] (kzt/kzt_label/tiptop_desc/offer/privacy/has_prodamus/has_transfer/transfer_rub/features) + guard [1,2,3,..] + back_url + style-override блок $course_id===N; (6) api/manual-payment.php $COURSE_PAY[id] (prodamus/prodamus_rub/transfer_rub/prefix). buy.php кнопка Prodamus теперь условна на has_prodamus. externalId для не-1 курса = "userId-courseId" (confirm-payment парсит). Реквизиты перевода общие (Сбербанк, Павел Г., +7 922 846-85-40, карта 2202 2023 5005 4791), сумма динамическая $C['transfer_rub'].
ОСТАЛОСЬ: курсы 4 (Эстетика педикюра), 5 (ESTHETIC NAILS), 6 (CONTENT NAIL) — ждём от Павла данные (описание/цены/YouTube-ссылки/фото) по каждому. Бэкапы боевых при деплое: *.bak-20260905-nbc.

### Курс 4 Эстетика педикюра — ГОТОВ И ЖИВОЙ (2026-09-05)
courses.id=4, 10 уроков (YouTube плейлист PLaJwaOqLJDRTgwiQIbyG8BmDxKmXTCmN7). Лендинг /esthetic-pedicure (esthetic-pedicure.php, шалфейно-зелёная палитра #6f9e6d/#46703f, цветной герой + светлые секции). Ассеты assets/esthped/{hero.jpg, works/work-1..9, inside/inside-1..4, reviews/review-1..4}. Оплата TipTop 12000₸ + перевод 2500₽ (без Prodamus). Юр: offer-ep/privacy-ep/contacts-ep.html. Карточка на главной .sys-card.ep + бейдж ликвидации. Программа с 🎁-бонусами и 🔥 Smart-техникой. buy.php $COURSES[4], guard [1,2,3,4], style-override course===4, manual-payment $COURSE_PAY[4] prefix EP. Бэкапы *.bak-20260905-ep.
ОСТАЛОСЬ: курсы 5 (ESTHETIC NAILS, 12000₸/2500₽), 6 (CONTENT NAIL, 9900₸/2000₽) — ждём данные Павла.

### Админка переведена на ДИНАМИКУ курсов (2026-09-05)
Была захардкожена под [1,2] — новые курсы 3-6 не появлялись (нельзя выдать доступ/управлять уроками). Пофикшено: все админ-файлы грузят курсы из БД `SELECT id,title FROM courses ORDER BY id` → $allCourses/$validCourseIds, авто-короткая подпись $courseShort (обрезка по ' — '/' - ', до 20-22 симв). Затронуты: admin/lessons.php (вкладки+badge), add-lesson.php+edit-lesson.php (селектор курса+guard), add-student.php+edit-student.php (чекбоксы доступа+intersect+foreach), students.php (фильтры+подписи, убрал хардкод $courseShort). dashboard.php/watch.php уже были динамическими (ЛК ученика + YouTube-плеер работают для любого курса). admin/index.php «по курсам» тоже уже динамический. Бэкапы admin/*.bak-20260905-multicourse. ВАЖНО: config.php НЕ читается SSH-юзером (ACL) → CLI-тест админки невозможен, проверять только через веб под логином; php -l чист, паттерн идентичен рабочему index.php.

### Курс 5 ESTHETIC NAILS — ГОТОВ И ЖИВОЙ (2026-09-05)
courses.id=5, 18 уроков (плейлист PLaJwaOqLJDRQCtvGmjWl9HHazUKm30gH0). Лендинг /esthetic-nails (esthetic-nails.php, сливово-розовая mauve #9b5c7d/#6d3a55). Ассеты assets/esthnails/{hero, works/work-1..9, inside/inside-1..2, reviews/review-1}. Только 1 отзыв → блок «Отзыв ученицы» (.review-single центрирован). 2 inside-фото. Оплата TipTop 12000₸ + перевод 2500₽ (без Prodamus). Юр offer-en/privacy-en/contacts-en. Карточка .sys-card.en. buy $COURSES[5] guard[1..5] override===5, manual-payment $COURSE_PAY[5] prefix EN. Бэкапы *.bak-20260905-en.
ОСТАЛОСЬ: курс 6 CONTENT NAIL (9900₸/2000₽) — ждём данные Павла. После него линейка 6 курсов закрыта.

### Курс 6 CONTENT NAIL — ГОТОВ. ЛИНЕЙКА ИЗ 6 КУРСОВ ЗАКРЫТА (2026-09-05)
courses.id=6, 13 уроков (плейлист PLaJwaOqLJDRQj33TfhczVdbEtdFFhadNO). Лендинг /content-nail (content-nail.php, индиго #5766d6/#363f9e). Контент-курс (съёмка/свет/Reels) → структура адаптирована: НЕТ блока отзывов (не было), «работы»→«Кадры из курса» (6 фото), «что внутри»→«Твой набор для контента» (3: реквизит/макролинза/камера). Ассеты assets/contentnail/{hero, works/1..6, inside/1..3}. Оплата TipTop 9900₸ + перевод 2000₽. Юр offer-cn/privacy-cn/contacts-cn. Карточка .sys-card.cn. buy $COURSES[6] guard[1..6] override===6, manual-payment $COURSE_PAY[6] prefix CN. Бэкапы *.bak-20260905-cn.

ИТОГ ЛИНЕЙКИ (6 курсов на fp.babichnail.online, главная = развилка 6 карточек):
1 Фруктовый педикюр (оранж, /frutped) · 2 Light System (золото, /light-system) · 3 NO BORDER+COLOR (тёмн.бирюза, /noborder, 29ур) · 4 Эстетика педикюра (шалфей, /esthetic-pedicure, 10ур) · 5 ESTHETIC NAILS (слива, /esthetic-nails, 18ур) · 6 CONTENT NAIL (индиго, /content-nail, 13ур).
Курсы 3-6: видео YouTube, оплата TipTop+перевод (без Prodamus), ликвидация −70%/5 дней, свои юр.страницы (-nb/-ep/-en/-cn), свои палитры. Все лендинги — клоны шаблона light-system.php с reveal-анимациями + карусель. Админка динамическая (см. запись про динамику). Слоты course_id заняты 1-6.

## Внешний аудит сайта (получен 2026-09-15, датирован 2026-09-10)
Павел прислал сторонний аудит `research/babichnail-audit-2026-09-10.md` — 18 задач BN-01..BN-18 (P0/P1/P2). Я перепроверил на живом: ПОДТВЕРЖДЕНО — payment-success.php показывает «успех/доступ выдан» при прямом открытии без оплаты (BN-01), payment-fail.php ретрай всегда на /buy.php=Fruit (BN-02), payment-security.php шаблон про «товарный вид»+«___ рабочего дня» (BN-03), title всех 6 buy-форм = Fruit Pedicure (BN-05), meta description только у frutped (BN-16). Аудит внешний — не знал, что серверная часть надёжна (webhook подпись, user_courses, ручное подтверждение) — чинить надо ВИТРИНУ.
ПЛАН (отправлен Павлу msg 1398): Этап1 P0 = success по контексту заказа + fail с сохранением курса + переписать payment-security под цифровую услугу + title форм. Этап2 = meta/OG, цены в карточках каталога, label/autocomplete, CLS No Border (0.186, section.learn), размеры изображений. Этап3 (нужны данные Павла) = дата окончания акции «5 дней»/убрать срочность, блок преподавателя, условия возврата, слово «ликвидация». Жду «го» на Этап 1.

## Аудит-доработки: Этапы 1-2 ВЫПОЛНЕНЫ (2026-09-15)
ЭТАП 1 (P0, bak-20260915-p0): payment-success.php переписан — 3 состояния (success только если payments.transaction_id найден со status=completed, курс из платежа; pending при tid без подтверждения; neutral без tid), noindex; payment-fail.php — все 6 курсов, retry=/buy.php?course=N, без контекста «Выбрать курс», добавлено «не оплачивайте повторно», noindex; payment-security.php переписан под цифровую услугу (без «товарного вида»/«___ дня», доступ ≤24ч, возврат по оферте, без PCI-обещаний); buy.php title=<?=$C['brand']?>; frutped футер: «Успешная оплата»→«Безопасность платежей».
ЭТАП 2 (P1, bak-20260915-p1): каталог index.php — категории вместо «Система/Курс N» (Педикюр·обработка стоп / Коррекция вросшего ногтя / Маникюр·верхние формы / Педикюр·эстетика / Маникюр·натуральные ногти / Контент·продвижение), цены .sys-price на карточках, футер-ссылки; все 7 страниц: meta description + canonical + OG (+og:image у 4 новых); buy.php форма: label for/id + autocomplete + email-подсказка; CLS-фикс: .hero-visual aspect-ratio (nb 563/1000, ep 3/4, en 591/1000, cn 4/5) + object-fit:cover; lazy loading="lazy" decoding="async" на галереях/inside/reviews всех лендингов (60+ img); <main> на всех страницах вкл. frutped.
ЭТАП 3 (ждёт данных Павла): дата окончания акции/убрать «5 дней», блок преподавателя, слово «ликвидация», условия возврата детально. BN-06 (вход/восстановление пароля), BN-07 (общая шапка) — не делал, обсудить приоритет.

### Курс 7 БОНДИРОВАНИЕ НОГТЕЙ — ПРЕДЗАПИСЬ, ЖИВОЙ (2026-09-22)
courses.id=7 «Бондирование ногтей» 45000.00, БЕЗ уроков (предзапись, старт 1 ноября — уроки зальём к старту). Лендинг /bonding (bonding.php) — ЕДИНСТВЕННЫЙ СВЕТЛЫЙ в линейке: крем #f4efe6 + мокко-шоколад #7c584b/#5a3f35 (было хаки-золото, Павел попросил убрать золото 22.09 — пересекалось с Light System) + пыльная роза #b98b84/#9d6f68, серифы Playfair Display. Палитра взята из фирменных промо-слайдов Марии (вшиты в лендинг как слайды «О системе»). Механика предзаписи: бейдж «Предзапись со скидкой · старт 1 ноября», цены с зачёркиванием 45 000₸/8 000₽ вместо 60 000₸/11 000₽ (−25%), пометка «уроки откроются 1 ноября» на лендинге и в buy-features; оферта offer-bd §2.3 доступ с 01.11.2026, §3.1 12 мес с момента старта. Ассеты assets/bonding/{hero.jpg, works/work-1..10.jpg, inside/inside-1..4.jpg}. Программа 7 тем + 2 бонуса (аппаратный маникюр без режущих; съёмка «голых ногтей»), техника «батута». Оплата: TipTop 45000₸ + перевод РФ 8000₽ (без Prodamus). Интеграция: index.php карточка .sys-card.bd (мокко-градиент, бейдж .sys-preorder роза) в категории «Маникюр · натуральные ногти»; buy.php guard[1..7] + $COURSES[7] + override===7 + back_url /bonding; api/manual-payment.php $COURSE_PAY[7] prefix BD; payment-success/fail COURSE_INFO[7] acc #5a3f35; .htaccess ^bonding. Юр: offer-bd/privacy-bd/contacts-bd.html. Бэкапы на beget: *.bak-20260922-bd. Всё проверено live (200 + цены/валюты). ПЕРЕД 1 НОЯБРЯ: залить уроки курса 7 (получить у Марии плейлист), проверить открытие доступа.

### Предзапись без оплаты для курса 7 — ЖИВАЯ (2026-09-22, вечер)
Павел: «форма предзаписи — если не хочет платить сейчас; после старта напоминание; в админке отдельный раздел, два списка кто оплатил до старта и после». Реализация: таблица `preorders` (id, course_id, first/last_name, email, phone, status enum waiting/paid/cancelled, utm varchar400, reminder_sent_at, paid_at, created_at; UNIQUE email+course). API `api/preorder.php` (POST JSON, только course=7, дедуп по email → ok без дубля, письмо клиенту «цена 45 000₸ закреплена, напомним перед 1.11» + уведомление админу). Лендинг: кнопка «Предзапись без оплаты» в герое (.hero-cta-row) и в price-box, модалка .pre-modal (имя/email/телефон → fetch → «Вы в списке»), utm из sessionStorage fp_utm уходит в заявку. Админка `admin/preorders.php`: три списка (⏳ Ждут оплаты / ✅ Оплатили до старта 1.11 / 🎉 После старта), синк оплат по email через payments completed ИЛИ user_courses active (UPDATE JOIN при каждом открытии), кнопка «Отправить напоминания» (confirm; письмо со ссылкой buy.php?course=7 и закреплённой ценой; помечает reminder_sent_at). Пункт «⏳ Предзапись» добавлен в сайдбар ВСЕХ админ-страниц (+ заодно «Источники», которого не было в старых навах). utm.js: /bonding добавлен в PREFIXES. Попутный фикс: из email-строки sendAdminPendingSignup в buy.php убран случайно вшитый sed'ом тег utm.js. Миграция таблицы: одноразовый tokenized-скрипт через HTTP (config.php читается только PHP-FPM), удалён. Бэкапы: *.bak-20260922-pre. E2e: оформление/дедуп/валидация/чужой курс 400/админ-редирект — всё проверено, тест-строка удалена.
**TODO к 1 НОЯБРЯ (напомнить Павлу самому!):** (1) поднять цену на лендинге/buy до 60 000₸/11 000₽ для новых, предзаписавшимся сохранить 45 000₸ (механику согласовать: например buy-ссылка из письма-напоминания с токеном или ручная проверка по списку); (2) залить уроки курса 7 (плейлист от Марии); (3) Павел жмёт «Отправить напоминания» в /admin/preorders.php.
ФИКС 22.09 вечер: /admin/preorders.php у Павла давал 500 — collation-конфликт: старые таблицы fp = utf8mb4_general_ci, а preorders создана с utf8mb4_unicode_ci → JOIN по email падал «Illegal mix of collations». Вылечено ALTER TABLE preorders CONVERT TO ... utf8mb4_general_ci. ПРАВИЛО: новые таблицы в БД fp создавать с COLLATE utf8mb4_general_ci. Также: php-cli на beget = 5.6 (php -l ругается на ??/стрелки) — линтовать локально, FPM на сайте = 8.3.

### Соц-страница предзаписи /predzapis — ЖИВАЯ (2026-09-23)
Павел: «доп страничка для соцсетей с формой предзаписи, в том же стиле, на сайте её не будет, ссылку оставляем под постом». Реализация: bonding-pre.php → маршрут ^predzapis (.htaccess). Компактная mobile-first (max-width 560px): бренд-строка, фирменный слайд hero.jpg, бейдж −25%, выжимка программы (5 пунктов), цена 45 000₸/8 000₽ с зачёркнутой, ИНЛАЙН-форма предзаписи (тот же api/preorder.php, course=7, utm из fp_utm) + ссылка «Оплатить со скидкой» на buy. Нигде не линкуется с сайта, robots noindex,nofollow; OG-теги с hero.jpg для превью в соцсетях; внизу ссылка на полный /bonding. UTM-ссылки под посты выданы Павлу (instagram post/stories, telegram — campaign=bonding). Prodamus для курса 7 ОТМЕНЁН Павлом 23.09 («большая комиссия сейчас») — оставляем только перевод на карту РФ 8 000₽. В коде ничего менять не пришлось (has_prodamus и так false). Если Павел передумает — схема: ссылка из кабинета → buy.php $COURSES[7] has_prodamus=true + $COURSE_PAY[7]['prodamus'].
